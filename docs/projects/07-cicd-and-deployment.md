...
### Phase 1.5 — 認証の CPU 制限問題解決（緊急対応）

Step 3 の動作確認中、Better Auth の password hashing が Workers Free の **10ms CPU 制限を超え**、signup / login が機能しないことが判明した。Phase 1b の workflow 化に進む前に、まずこの問題を解決して「機能的に動くアプリ」の状態を作る。

#### Step 4.5. backend 認証の CPU 時間制限を Cloudflare Workers Free で解消する

主な変更対象（いずれかを選択）:

- Better Auth の `hash` option を Web Crypto API の PBKDF2 ベースに差し替え
- signup / login だけ別 Worker や Queue 経由に分離
- Cloudflare Workers Standard plan ($5/月) への upgrade（ADR 008 の前提に戻すが、商用化前の予算判断が必要）

目的:

- signup / login が production で動作する状態にする
- ADR 008 の決定（Cloudflare Workers 採用）を維持しつつ、Workers Free 運用を継続できる方向を模索する

動作確認:

- production `workers.dev` URL で signup / login / logout が成功する
- CPU 時間が制限内に収まる（`wrangler tail` で測定）
- 既存セッションが壊れない（hashing アルゴリズム変更時は移行戦略も検討）

参考:

- 関連 Issue: #123
- ADR 008 Section 2.2（Cloudflare Workers の CPU 制限）
- ADR 008 Section 9（コスト試算は Standard plan 前提だが、商用化前は Free 維持を優先）
