...
3.  **ドキュメントバージョンの更新:** 修正を反映したら、変更対象の `api-[DOMAIN].md` 内にあるメタ情報より、**「ドキュメントバージョン」**を必ず更新します。
  - **MAJORバージョン** (`v1.x` → `v2.0`)：**後方互換性のない破壊的変更**（例：フィールドの削除、必須化）。
  - **MINORバージョン** (`v1.0` → `v1.1`)：**後方互換性のある変更**（例：任意フィールドの追加、軽微な修正）。
  - 変更内容はPull Requestに必ず記載してください。