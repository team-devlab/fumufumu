...
- 通知実行方式（MVP採用）: **パターンE（A案 + D案ハイブリッド）**
  - 理由: 通常系はA案（`ctx.waitUntil`）で自動化し、失敗時のみD案（`resend`）で回復することで、
    `fail-open` を維持しながら日次運用コストを下げられるため。
  - 本RFCではこの方針を今回の推奨案として採用する。
  - 導入順は「D案の送信状態管理/再送導線を先に整備」→「A案の正常系自動送信を有効化」とし、
...
現状、運営による承認APIの実行は手動オペレーションで運用されている。
...
1. パターンA: 承認API + `ctx.waitUntil` 非同期送信
- 仕組み:
  - 承認APIは判定更新後、HTTPレスポンスを先に返す。
  - 返却後に `ctx.waitUntil` でメール送信処理を実行する。
  - 送信結果（成功/失敗）は `content_checks` に記録し、失敗時は `resend` で回復する。
- 構成図:
```mermaid
sequenceDiagram
  participant Admin as 運営クライアント
  participant API as 承認API（Worker）
  participant DB as D1
  participant Mail as メールProvider

  Admin->>API: 承認リクエスト（approve/reject）
  API->>DB: 判定結果を更新
  DB-->>API: 更新OK
  API-->>Admin: 200 OK（先に応答）
  Note over API: ctx.waitUntil(...) で非同期送信
  API->>Mail: メール送信
  Mail-->>API: 成功/失敗
  API->>DB: 通知結果を記録
```
...
- デメリット:
  - Worker実行時間・リトライ戦略の設計が必要。

2. パターンB: Outboxテーブル + Scheduled Worker（DBポーリング）
...
3. パターンC: Queue Producer/Consumer
- 仕組み:
...
- 仕組み:
  ...
  - 同時実行対策として、送信対象の更新時に楽観的ロック（例: `updated_at` / `version` 条件付き更新）を入れ、二重送信を抑止する。
  - コマンド側でも簡易的な多重起動防止（ロックファイル/重複実行チェック）を行う。
- メリット:
...
- コスト観点:
  - インフラ費用は比較的小さく抑えやすい（Queue/Cronの常時実行コストがない）。
  - 一方で、日次の実行・確認・再送判断に人的運用コストが継続的に発生する。
  - 通知件数が増えるほど、手動実行/確認の工数が増加しやすい。

- API責務の整理（判断コストと作業コストの分離）:
  - 承認APIの本質は「内容が適切かを判断する」意思決定プロセス（人が担う価値業務）。
  - 一方、通知送信コマンド実行は機械的な反復作業（トイル）であり、将来的には自動化対象とする。
  - 本RFCでは、価値業務（承認判断）と作業業務（通知実行）を分けてコスト評価する。
...
- 運用（暫定案）:
  ...
  - 同時実行防止ルール: 実行担当者は毎回1名に固定し、実行開始/完了を運営チャネルで明示する（Runbookに記載）。

- 段階移行方針（最速リリース + 運用負荷低減）:
  - Step 1: まずD案（手動送信 + 再送）を実装し、リリースブロックを解除する。
  - Step 2: 正常系送信のみを承認API内の `ctx.waitUntil` で自動実行する（レスポンスは先に返す）。
  - Step 3: 失敗時の回復導線はD案の `resend` を継続利用し、`fail-open` を維持する。
  - これにより、日々の手動オペレーションを削減しつつ、安全性と実装速度を両立する。

- 比較メモ（B/Cとの比較例）:
  - B案比: 自動実行基盤が不要な分、導入は軽いが、送信自動性は下がる。
  - C案比: Queue導入コストは抑えられるが、遅延/人依存リスクは相対的に高い。
  - 工数感: 初期実装は小さいが、件数増加時はB/Cへの移行検討が必要になりやすい。
...
5. パターンE: A案 + D案（ハイブリッド）
- 仕組み:
  - 通常系はA案を採用し、承認API更新後に `ctx.waitUntil` で非同期送信を実行する。
  - 送信失敗時は `content_checks` に失敗情報を記録し、D案の `resend` コマンドで回復する。
  - これにより、承認作業は「承認するだけ」で完結し、手動作業は失敗時に限定される。
- 実装方針（MVP）:
  - Step 1: 承認APIで `content_checks` の判定結果を更新する（ここは通知成否に依存させない）。
  - Step 2: 判定更新成功後に `ctx.waitUntil(sendMailAndRecord(...))` を実行する。
  - Step 3: 非同期処理内でメール送信（Resend HTTP API）を実行する。
  - Step 4: 送信成功時は `notified_at` を更新し、失敗時は `notify_last_error` を記録する。
  - Step 5: 失敗対象のみ `resend` コマンドで手動回復する。
- メール基盤に関する補足:
  - `ctx.waitUntil` は「非同期実行の仕組み」であり、メール配信基盤そのものではない。
  - MVPでは自前メールサーバーは構築せず、Resend など外部メールプロバイダのAPIを利用する。
  - そのため必要なのは「プロバイダAPIキー」「送信元ドメイン認証（SPF/DKIM）」である。
- メリット:
  - 日次運用の手動コストを削減しやすい。
  - `fail-open` を維持しつつ、回復手段（再送）を明確に持てる。
  - D案の運用資産（Runbook/再送導線）を活かして段階導入しやすい。
- デメリット:
  - `ctx.waitUntil` 側の失敗検知・ログ連携設計を先に固める必要がある。
- MVP適用メモ:
  - 「最初にD案のみ」ではなく、「最初からE案」で入れると、運用負荷を抑えた立ち上がりが可能。
  - 実装順は `送信状態管理` -> `ctx.waitUntil` 送信 -> `resend` 回復導線 の順が安全。
...
- 既存スタックとの親和性:
  - Cloudflare Workers + D案（手動コマンド）と組み合わせやすい
