...
- `sort_order`は、バッファを持たせるために10,20,30...のように区切りを設ける
...
- 下書きに関しては、タグの選択を無しでもOKとする
- 公開投稿時、もしくは下書き投稿から公開時、アプリケーションロジックでタグが選択されているかを必須で確認する
  - FE,BE両方でチェック
...
- タグに相談のcount(全件走査)を入れるためキャッシュに関しては、本実装で導入する

#### 理由
- Cloudflare D1特有の課金・制限体系的に無料枠のRows read(行数)が500万/日であるが、シュミレーションによると仮に相談件数が1000件になった場合、無料枠を使い切る可能性がある。相談が仮に100件であったとしても、一人当たりのユーザーが一覧、詳細、投稿画面を行き来した場合、タグを切り替えまくり検索をすると、課金対象となる可能性が高い。
