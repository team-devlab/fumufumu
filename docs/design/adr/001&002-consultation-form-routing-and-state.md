...
> 補足 (ADR 012): 本 ADR はデータ保持を in-memory と決定したが、実装は後に「リロード時の入力保持」を目的として sessionStorage persist を採用した（コミット `05182c1`, 2026-02-07）。この逸脱は本 ADR に未反映のままだったため、ADR 012 で persist を正式な決定として追認・整理した。
...
> 補足 (ADR 012): 実装詳細方針 #3 は編集機能を「共有ストア + setInitialData」で想定していたが、sessionStorage persist 前提での作成⇄編集の状態混線を避けるため、ADR 012 で「編集専用ストアの新設」に更新した。
