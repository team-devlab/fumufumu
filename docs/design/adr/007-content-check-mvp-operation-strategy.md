...
- 運営APIを `reviews` 1本に統合する案
  - 却下理由: 相談/助言でレスポンスと運用観点が異なり、初期運用で複雑化しやすい
...
fumufumuでは、相談（consultation）と助言（advice）をユーザーが即時投稿できるため、公開前に最低限の内容確認を行わないと、ガイドライン違反投稿や不適切な表現がそのまま表示されるリスクがある。  
...
| 助言対応 | `consultation/advice` とも同一運用が可能 | 関連整合（どの相談への助言か）の扱いが複雑化 |
...
- 1投稿（相談/助言）につき投稿チェックレコードは1件のみとし、再チェック時は同一レコードを更新する。
...
- 理由:
  - レスポンス項目が異なるため（相談は `title + body`、助言は `body` 中心、親相談情報の参照が必要）。
  - チェック観点が異なるため（相談本文の妥当性と、助言としての適切性は評価軸が異なる）。
...
- 主要API:
  - `GET /admin/content-check/consultations?view=summary`（未チェック一覧取得）
  ...
  - `GET /admin/content-check/advices/find?advice_ids=301,302,999`（ID指定の複数助言取得）
...
3. プロフィールのアドバイス一覧カードに `投稿チェック中` を表示する。（投稿チェック中のものは、他の一覧や相談詳細の助言一覧では表示されない。）
4. `pending` 状態のカードは助言詳細へ遷移不可にし、`運営が投稿チェック中です` 文言を表示する。
5. `approved` に更新後は、同カードから助言詳細へ遷移可能にする。（相談詳細の助言一覧にも表示される。）

### APIリクエスト/レスポンス（MVP）

#### 1) 未チェック一覧取得

- Endpoint: `GET /admin/content-check/consultations?view=summary`
- Request:
  - Query:
    - `view`: `summary`（省略時も `summary`）
- Response:
...
- Endpoint: `GET /admin/content-check/consultations?view=detail&ids=101,205,999`
- Request:
  - Query:
    - `view`: `detail`（必須）
    - `ids`: カンマ区切りのID文字列（例: `101,205,999`）
...
```json
{
  ...
  "missing_ids": [999],
  "non_pending": [
    { "id": 205, "current_status": "approved" }
  ]
}
```
...
- 挙動:
  ...
  - `approved/rejected` の相談IDは `non_pending` 側で返す
  - `ids` の件数増加によりURL長の制約が懸念される場合は、`POST /admin/content-check/consultations/find` への切り替えを行う
...
- 挙動:
  - `content_checks.status = pending` の助言のみ `advices` に含める
  - `approved/rejected` の助言IDは `not_found_ids` 側で返す
...
- 権限方針（MVP）:
  ...
  - 対象は以下6APIを含む運営用API全体とする。
    - `GET /admin/content-check/consultations?view=summary`
    - `GET /admin/content-check/consultations?view=detail&ids=...`
